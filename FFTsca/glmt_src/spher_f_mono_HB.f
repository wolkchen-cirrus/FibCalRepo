C  Modified by H.Ballington on 09/09/2022 to loop through sizes. Sizes

C  given as a list in an input file 'input.txt'. Scattering cross

C sections are given in output file CEXT.print
...
C  Modification for MONODISPERSE spheres, radius entered interactively    

                                                                     

C  Last modified 11/20/2000

C  Monodisperse spheres mofification 10/2/2006                                            
...
C   any damages that may result from the use of the program. 
     ...
     &       BET2(NPL),MRR,MRI,LAM         
      ...
      INTEGER*8 NLINES, LOOP                                 

      character(100) cwd, cwd2, cross_file
      character(100) jobIDString
      logical result

      result = getcwd(cwd)

      open(11, file = "jobID.txt")

      read(11,*)jobIDString

      close(11)

      cwd2 = trim(trim(cwd)//"/"//"job"//trim(jobIDString)//"/CEXT_MIE")

      open (6, file='spher.print')

      open (10, file='spher.write')

      open (12, file=cwd2) ! output file for scat. cross sections
...
C   399 FORMAT (I4)
      ...
      open(11, file = 'input.txt',status = 'old')
      open(1212, file = 'mie_params.txt',status = 'old')
...
C       WRITE(*,*) 'Wavelength=',LAM,' (um), refr. index=',MRR,',',MRI

    !  WRITE(*,*) 'Enter radius in microns: '
...
C  PRINTOUT   **********************************************************        
  ...
  599 FORMAT(f5.1,f18.6)   

      WRITE (12,599)AA,CEXT                                             
